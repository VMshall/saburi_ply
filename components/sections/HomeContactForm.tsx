...
export function ContactForm() {
  ...
  const handleInquiryTypeChange = (selectedOption: { value: string; label: string } | null) => {
    setFormData((prev) => ({ ...prev, inquiryType: selectedOption ? selectedOption.value : "" }));
    if (error) setError(null);
  };

  const handleProductTypeChange = (selectedOption: { value: string; label: string } | null) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Same-origin proxy → forwarded to apiv2 server-side.
      ...
      // Redirect to thank you page with context carried via query params.
