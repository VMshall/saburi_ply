...
/**
 * Gallery grid + lightbox (client island). Ported from client/pages/Gallery.jsx — the grid
 * needs onClick handlers, the lightbox holds open/index state, keyboard nav, and toggles
 * document.body.style.overflow, so the interactive part lives here while app/gallery/page.tsx
 * stays a server component that exports metadata. Text, Tailwind classes, and image paths are
 * kept verbatim; the legacy <LazyImage> grid thumbnails become plain <img> (default objectFit
 * "cover"), and the lightbox <img> is preserved as-is.
 */
...
export function GalleryGrid() {
  ...
  const groupedByTitle = filtered.reduce(
    ...
    { order: [] as string[], map: new Map<string, GalleryItem[]>() },
  );
